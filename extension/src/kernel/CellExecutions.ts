...
export interface Interface {
  ...
  ) => Option.Option<NotebookExecutions>;
  readonly staleChanges: Stream.Stream<
    HashMap.HashMap<NotebookId, HashSet.HashSet<NotebookCellId>>
  >;
}

/** Owns one ordered collection of cell runs per exact notebook session. */
export class Service extends Context.Service<Service, Interface>()(
  "@marimo/CellExecutions",
) {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      code.workspace.notebookDocumentChanges.pipe(
        Stream.filter(
          (event) =>
            ...
            event.contentChanges.some((change) => change.addedCells.length > 0),
        ),
    ...
    const makeNotebook = Effect.fn("CellExecutions.makeNotebook")(function* (
      ...
      yield* Effect.addFinalizer(() =>
        session.close.pipe(
          ...
          Effect.ensuring(
            SubscriptionRef.set(staleRef, HashSet.empty<NotebookCellId>()).pipe(
              Effect.andThen(
    ...
    const invalidate = Effect.fn("CellExecutions.invalidate")(function* (
      notebookId: NotebookId,
    ) {
      yield* notebooks.get(notebookId)?.executions.invalidate ?? Effect.void;
    });
    const find = (document: vscode.NotebookDocument) => {
    ...
    return Service.of({
      ...
      invalidate,
      find,
      staleChanges: SubscriptionRef.changes(allStaleCells),
    });
  }),
);

export const defaultLayer = layer.pipe(
  Layer.provide([NotebookDocumentSessions.layer, NotebookEditorRegistry.layer]),
);
