...
function getFileFormattingOptions(
  ...
  return Effect.map(
    code.workspace.getConfiguration("files", doc),
    (cfg) => ({
      trimTrailingWhitespace:
        cfg.get<boolean>("trimTrailingWhitespace") || undefined,
      trimFinalNewlines:
        cfg.get<boolean>("trimFinalNewlines") || undefined,
      insertFinalNewline:
        cfg.get<boolean>("insertFinalNewline") || undefined,
    }),
  );
}
...
export const registerDocumentRangeFormattingProvider = Effect.fn(
  function* (
    sel: vscode.DocumentSelector,
    client: NotebookLspClient,
  ) {
    if (!client.serverInfo.capabilities.documentRangeFormattingProvider)
      return;
    const code = yield* VsCode;

    yield* code.languages.registerDocumentRangeFormattingEditProvider(
      sel,
      {
        provideDocumentRangeFormattingEdits: Effect.fn(function* (
          doc,
          range,
          opts,
        ) {
