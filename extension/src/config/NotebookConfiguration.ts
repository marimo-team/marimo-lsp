...
export interface Interface {
  readonly get: Effect.Effect<MarimoConfig, Error>;
  readonly update: (
    partialConfig: Record<string, unknown>,
  ) => Effect.Effect<MarimoConfig, Error>;
  readonly invalidate: Effect.Effect<void>;
  readonly changes: Stream.Stream<Option.Option<MarimoConfig>>;
}

/** Configuration state for one open notebook document session. */
export class Service extends Context.Service<Service, Interface>()(
  "@marimo/NotebookConfiguration",
) {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const update = Effect.fn("NotebookConfiguration.update")(function* (
      partialConfig: Record<string, unknown>,
    ) {
      return yield* mutate(
        Effect.gen(function* () {
