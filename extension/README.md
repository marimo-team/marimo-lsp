...
| --------------------------------------- | --------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `marimo.lsp.path`                       | `array`   | `[]`             | Path to a custom `marimo-lsp` executable, e.g., `["/path/to/marimo-lsp"]`. Leave empty to use the bundled WASM server by default, or the uv-based server when `marimo.experimental.wasmLsp` is disabled. |
| `marimo.experimental.wasmLsp`           | `boolean` | `true`           | Run the bundled marimo language server in Pyodide instead of provisioning a Python environment.                                                                                                          |
