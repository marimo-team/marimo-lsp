...
class PullRequest(msgspec.Struct, rename="camel"):
...
def is_version_commit(commit: Commit) -> bool:
    """Check if commit is a version bump commit (e.g., 'v0.7.3')."""
    # Match patterns like "v0.7.3", "v0.7.3 (pre-release)", etc.
    return bool(re.match(r"^v\d+\.\d+\.\d+", commit.message))
...
def categorize_entries(
    ...
    for entry in entries:
        ...
        if is_version_commit(entry.commit):
            continue
...
def format_entry(entry: CategorizedEntry) -> str:
    ...
    if entry.pr:
        ...
        return f"* {title} ([#{entry.pr.number}](https://github.com/marimo-team/marimo-lsp/pull/{entry.pr.number}))"
...
def generate_release_notes(since_tag: str, current_tag: str) -> str:  # noqa: C901, PLR0912
    ...
    if categories["breaking"]:
        ...
        for entry in categories["breaking"]:
            notes.append(format_entry(entry))  # noqa: PERF401
        notes.append("")
    ...
    notes.append(
        f"\n**Full Changelog**: https://github.com/marimo-team/marimo-lsp/compare/{since_tag}...{current_tag}"
    )
