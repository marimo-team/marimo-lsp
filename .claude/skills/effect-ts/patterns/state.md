...
Repo convention: prefer `Effect.fnUntraced(function* (a) { ... })` for the inner update body (avoids spawning one span per atomic update); reserve `Effect.fn("namespace.op")` for the *outer* named operation. Several call sites in `ControllerRegistry.ts` currently use `Effect.fn` for the inner body — see "What to avoid" below.
...
`DatasourcesService.ts:408` returns `connectionsRef.changes` directly (no `Stream.changes`); that's fine for that surface because the wrapping `DatasourcesView` debounces refreshes downstream. By contrast, `PackagesService.ts:400` and `PackagesService.ts:409` also return raw `.changes` and likely should pipe through `Stream.changes` for parity with `VariablesService` — flagged in "What to avoid."
...
- **Forgetting `Stream.changes` after `ref.changes`** — every `set`/`update` emits, even no-op updates. `PackagesService.ts:400` and `PackagesService.ts:409` expose raw `.changes`; `VariablesService` and `MarimoConfigurationService` pipe through `Stream.changes`. Possible bug — flagged.
- **`Effect.fn("...")` inside `SynchronizedRef.updateEffect` callbacks** — each atomic update gets its own span. Prefer `Effect.fnUntraced(function* (a) { ... })` for the inner update; trace the outer named operation instead. `ControllerRegistry.ts:79,293,346` currently use `Effect.fn`.
...
Anti-patterns to flag (spotted while writing this):

1. `extension/src/panel/packages/PackagesService.ts:400,409` returns raw `ref.changes` without `Stream.changes`, while peer services (`VariablesService`, `MarimoConfigurationService`) all dedup. Likely produces redundant tree-view refreshes on no-op updates.
2. `extension/src/kernel/ControllerRegistry.ts:79,293,346` uses `Effect.fn(function* (map) { ... })` for the inner `SynchronizedRef.updateEffect` body. Repo convention is `Effect.fnUntraced` there — the outer operation already has a named span (`ControllerRegistry.refresh`, `ControllerRegistry.createOrUpdate`, `pruneStaleControllers`), so the inner `Effect.fn` just produces span noise per atomic update.
